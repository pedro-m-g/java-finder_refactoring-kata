...
public class PersonAgeDiffCalculator {
	private final List<Person> people;

	public PersonAgeDiffCalculator(List<Person> people) {
		this.people = people;
	}
	...
	private List<PersonPair> peopleSetProduct() {
		...
		return firstElements.stream()
				.flatMap(firstPerson -> secondElements.stream()
						.filter(secondPerson ->
							people.indexOf(secondPerson) > people.indexOf(firstPerson))
						.map(secondPerson -> firstPerson.compareTo(secondPerson) < 0
